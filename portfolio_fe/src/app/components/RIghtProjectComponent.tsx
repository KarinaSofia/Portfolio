...
const RightProjectComponent: React.FC<LeftProjectComponentProps> = ({title, imageUrls, description, linkUrl, linkText }) => {
    const handleClick = () => {
        ...
        <div className="flex max-w-5xl rounded overflow-hidden shadow-lg">
            ...
            <div className="flex flex-col">
                {imageUrls.map((url, index) => (
                    <div key={index}>
                        <Image src={url} alt={`Project Image ${index + 1}`} width={200} height={125} layout="responsive" />
                    </div>
                ))}
            </div>
        </div>
...
export default RightProjectComponent;
